...
var _prebuffer_size := 3600 # ~75ms at 48kHz (v3.7 Low Latency)
var _target_playback_fill := 1800 # Keep 37.5ms in Godot's buffer

func _ready() -> void:
	print("[DesktopClient] CLIENT v3.7 (Low Latency)")
...
func _handle_audio_packet(adpcm_data: PackedByteArray) -> void:
	...
	# Auto-catchup: If buffer is huge (>200ms), drop older samples to reduce latency
		if _audio_buffer.size() > 9600:
			_audio_buffer = _audio_buffer.slice(_audio_buffer.size() - 4800)
